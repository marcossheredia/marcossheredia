...
**Focus:** reliability, event processing, system integration and clear operational interfaces.

`Java` `Spring Boot` `Vue` `TypeScript` `SQL` `Docker`
...
**Focus:** local inference, traceability, fallback mechanisms and robust validation.

`Python` `PaddleOCR` `OpenCV` `FastAPI` `Docker`
...
**Focus:** usable administration panels, scalable backend design and maintainable business logic.

`Spring Boot` `Vue` `TypeScript` `PostgreSQL` `Docker`
...
**Focus:** live metrics, native desktop UX and local hardware integration.

`C#` `.NET` `WPF` `LibreHardwareMonitor`
